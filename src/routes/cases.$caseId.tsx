...
import { InvestigationGraph } from "@/components/watson-board/InvestigationGraph";
import { HypothesisPanel } from "@/components/watson-board/HypothesisPanel";
import { ContradictionPanel } from "@/components/watson-board/ContradictionPanel";
...
import { AutopsyPanel } from "@/components/watson-board/AutopsyPanel";
import { Copilot } from "@/components/watson-board/Copilot";
// import { WhatIfPanel } from "@/components/watson-board/WhatIfPanel";
...
export const Route = createFileRoute("/cases/$caseId")({
  head: ({ params }) => ({
    meta: [
      { title: `Case ${params.caseId} — Watson-Board Investigation Workspace` },
      {
        name: "description",
        content: "Living evidence canvas, AI hypotheses, autopsy & timeline replay.",
      },
    ],
  }),
    ...
    <Shell>
      <div className="space-y-4 p-5">
        ...
        <div className="glass-strong flex flex-wrap items-center gap-4 rounded-xl px-5 py-4">
          <Link
            to="/"
            className="grid h-9 w-9 place-items-center rounded-md border border-border/60 hover:text-primary"
          >
            ...
            <div className="flex items-center gap-2 text-[11px] font-mono text-muted-foreground">
              ...
              {c.flagged && (
                <span className="flex items-center gap-1 text-danger">
                  <ShieldAlert className="h-3 w-3" /> AI Flagged
                </span>
              )}
            </div>
        ...
        <div
          className={`grid grid-cols-1 gap-4 ${tab === "graph" ? "xl:grid-cols-[1fr_360px]" : ""}`}
        >
          {/* Center stage */}
          <div className="space-y-4">
              ...
              <div className="glass relative h-[640px] overflow-hidden rounded-xl">
                <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-center justify-between border-b border-border/40 bg-background/40 px-3 py-2 backdrop-blur">
                  <div className="text-[11px] uppercase tracking-widest text-muted-foreground">
                    Living Evidence Canvas · 15 nodes · 16 edges
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1">
                      <span className="h-2 w-2 rounded-full bg-danger" /> suspicious
                    </span>
                  </div>
                </div>
                <ReactFlowProvider>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div className="glass rounded-xl p-4">
                ...
                <div className="space-y-1.5">
                  {evidenceVault.slice(0, 5).map((e) => (
                    <div
                      key={e.id}
                      className="flex items-center gap-2 rounded-md border border-border/40 bg-secondary/20 px-2.5 py-1.5 text-xs"
                    >
                      <div className="font-mono text-[10px] text-muted-foreground">{e.id}</div>
                      <div className="flex-1 truncate">{e.name}</div>
                      <div className="hidden truncate text-[10px] text-muted-foreground sm:block max-w-[160px]">
                        {e.ai}
                      </div>
                      <div className="h-1 w-12 overflow-hidden rounded bg-secondary">
                        <div
                          className="h-full bg-gradient-to-r from-warn to-danger"
                          style={{ width: `${e.priority}%` }}
                        />
                      </div>
              ...
              <div className="glass rounded-xl p-4">
                <div className="mb-2 text-[11px] uppercase tracking-widest text-muted-foreground">
                  Cross-Case Memory · Similar Patterns
                </div>
                  ...
                  {similarCases.map((s) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="font-mono text-[10px] text-neon-2">
                          {s.similarity}% match
                        </div>
                      </div>
            ...
            <div className="glass rounded-xl p-4">
              ...
              <p className="text-sm leading-relaxed text-foreground/90">
                <span className="font-mono text-[10px] text-primary">Watson-Board ▸ </span>
                The victim arrived near Chennai Central Station at <b>20:14</b>. The suspect's
                device appeared within tower overlap <b>6 minutes later</b>. A UPI transfer of{" "}
                <b>₹40,000</b> from victim to S-118 was logged at 20:22, suggesting a planned
                meeting. CCTV-0412 captured an altercation at <b>20:42</b>, after which the victim's
                phone went silent at 20:51. Autopsy estimates time of death between{" "}
                <b>19:30 and 21:00</b>, consistent with the digital trail.
              </p>
            </div>
          </div>
