...
export function WhatIfPanel({
  ...
  const dX = active.reduce(
    (a, id) => a + SCENARIOS.find((s) => s.id === id)!.delta.contradictions,
    0,
  );
          ...
          return (
            <button
              ...
              className={[
                ...
                on
                  ? "border-neon-2/50 bg-neon-2/10 text-foreground"
                  : "border-border/50 bg-secondary/30 text-muted-foreground hover:text-foreground",
              ].join(" ")}
            >
              <span>{s.label}</span>
              <span
                className={`h-3 w-3 rounded-full border ${on ? "border-neon-2 bg-neon-2" : "border-muted-foreground"}`}
              />
            </button>
          );
        })}
