...
const NODE_STYLES: Record<
  MiniNodeType,
  { border: string; bg: string; labelColor: string; typeTag: string }
> = {
  victim: {
...
const STEPS: ReplayStep[] = [
  ...
  },
  {
    id: 2,
    time: "19:42",
    title: "UPI Transfer to Suspect S-118",
    eventType: "financial",
    description: "₹40,000 transferred from victim to Vetri",
    confidence: 88,
    severity: "suspicious",
    aiInsight:
      "Financial link established. ₹40,000 UPI transfer from R. Suresh to Vetri (S-118) sets up financial motive 90 min before TOD.",
    nodes: [
  ...
  },
  {
    id: 3,
    time: "20:14",
    title: "CCTV Spots Victim at E-Gate 4",
    eventType: "cctv",
    description: "Victim captured at Chennai Central E-Gate 4",
    confidence: 95,
    severity: "normal",
    aiInsight:
      "Victim seen at E-Gate 4. CCTV-0412 timestamp verified. Consistent with movement trajectory from Triplicane.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    ...
    severity: "suspicious",
    aiInsight:
      "Both phones in same tower area. Victim and Vetri (S-118) within 500m of each other from 20:22–20:51. Proximity confirmed.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    id: 5,
    time: "20:42",
    title: "Altercation Captured",
    eventType: "cctv",
    description: "Physical altercation captured on CCTV-0412",
    confidence: 90,
    severity: "critical",
    aiInsight:
      "Victim and suspect co-located. Altercation captured in 6 clear frames at 20:42. Key evidence of physical confrontation.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    id: 6,
    time: "20:48",
    title: "Timestamp Anomaly Found",
    eventType: "anomaly",
    description: "CCTV-0418 clock drifts +6 min from master clock",
    confidence: 47,
    severity: "suspicious",
    aiInsight:
      "Possible timestamp manipulation. +6 minute drift detected. Could create false alibi window. Under forensic review.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    id: 7,
    time: "20:51",
    title: "Victim Phone Last Ping",
    eventType: "phone",
    description: "Last phone activity from victim's device",
    confidence: 84,
    severity: "suspicious",
    aiInsight:
      "Likely last phone activity. Victim's device goes dark at 20:51 — consistent with incapacitation during altercation window.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    ...
    severity: "normal",
    aiInsight:
      "Witness saw argument near station. Anandhi K. (stall owner) statement consistent with CCTV-0412. Reliable eyewitness.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    ...
    severity: "normal",
    aiInsight:
      "TOD window narrowed. Vitreous K+ 6.1 mmol/L + rigor mortis → TOD 19:30–21:00. Consistent with altercation at 20:42.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    id: 10,
    time: "22:14",
    title: "Vehicle Leaves Crime Area",
    eventType: "vehicle",
    description: "TN-09-AC-4421 spotted leaving 22:14",
    confidence: 72,
    severity: "suspicious",
    aiInsight:
      "Suspect vehicle identified. TN-09-AC-4421 linked to S-118 associate. Exits scene 73 min post estimated TOD.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    id: 11,
    time: "22:40",
    title: "DNA Match Confirmed",
    eventType: "forensic",
    description: "DNA Sample D-77 matches S-118 at 99.2%",
    confidence: 99,
    severity: "critical",
    aiInsight:
      "Strong biological evidence. STR profiling confirms DNA from occipital wound = Vetri (S-118) at 99.2%. Primary forensic link.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    id: 12,
    time: "23:02",
    title: "Witness #2 Reports Incident",
    eventType: "witness",
    description: "Auto driver places S-204 at scene at 22:10",
    confidence: 71,
    severity: "suspicious",
    aiInsight:
      "Second witness corroborates but contradicts. Places S-204 at scene — however S-204's phone shows no tower activity. Inconsistency flagged.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    id: 13,
    time: "00:15",
    title: "Toxicology Report Added",
    eventType: "forensic",
    description: "Diazepam trace detected in victim's blood",
    confidence: 82,
    severity: "critical",
    aiInsight:
      "Drug traces detected. Diazepam trace suggests pre-assault sedation. Consistent with S-118's background as drug supplier.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
  ...
  },
  {
    ...
    severity: "normal",
    aiInsight:
      "Motive hypothesis generated. Financial dispute over ₹40,000 likely escalated into premeditated assault. S-118 is primary suspect per AI model.",
    special: "ai-hypothesis",
  },
  {
    id: 15,
    time: "02:30",
    title: "Movement Reconstruction Complete",
    eventType: "analysis",
    description: "Victim and suspect routes reconstructed",
    confidence: 86,
    severity: "normal",
    aiInsight:
      "Routes overlap at E-Gate 4. Movement reconstruction confirms both victim and suspect converged at 20:42 — matching altercation time exactly.",
    special: "movement",
  },
  {
    id: 16,
    ...
    severity: "suspicious",
    aiInsight:
      "Witness timing conflicts with CCTV-0418. Statement conflict between Anandhi K. testimony and auto driver's timeline. Contradiction flagged.",
  ...
  },
  {
    ...
    severity: "critical",
    aiInsight:
      "Cause of death linked. Iron rod → head trauma → blood sample B-09 chain established. Weapon discarded 100m post-assault.",
    nodes: [
      { id: "v", typeKey: "victim", label: "VICTIM", sublabel: "R. Suresh" },
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <div className="text-[9px] text-slate-500 mt-0.5">
              Any format · timeline auto-generated on upload
            </div>
          </div>
          ...
          >
            <div>
              ...
              <div className="text-[9px] text-slate-500">
                {STEPS.length} events · 18-step investigation roadmap
              </div>
            </div>
            <button
              onClick={onDismiss}
  ...
  return (
    <div
      className={[
        ...
        active
          ? `${c.ring} ${c.bg} ${c.text} shadow-lg`
          : past
            ? "border-white/20 bg-white/5 text-slate-500"
            : "border-white/15 bg-transparent text-slate-600",
      ].join(" ")}
    >
      {number}
    </div>
...
function MiniNodeCard({ node }: { node: MiniNode }) {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 rounded-lg border px-2 py-1.5 min-w-[80px] max-w-[105px] shrink-0 ${s.border} ${s.bg}`}
    >
      <div className="shrink-0">{NODE_ICON[node.typeKey]}</div>
      <div className="min-w-0">
        <div
          className={`text-[8px] font-bold uppercase tracking-wider ${s.labelColor} leading-none mb-0.5`}
        >
          {node.label}
        </div>
        {node.sublabel && (
...
function EdgeConnector({
  ...
  return (
    <div className="flex flex-col items-center justify-center w-14 shrink-0 px-1">
      {label && (
        <span className="text-[7px] text-slate-400 text-center leading-tight mb-0.5 max-w-[52px] truncate">
          {label}
        </span>
      )}
  ...
  return (
    <div className="flex items-center gap-3 w-full">
      <div className="flex items-start gap-2 rounded-lg border border-fuchsia-500/40 bg-fuchsia-950/50 px-3 py-2 min-w-[160px]">
        ...
        <div>
          <div className="text-[8px] font-bold uppercase text-fuchsia-400 tracking-wider">
            AI Hypothesis
          </div>
      ...
      <div className="flex items-center gap-1.5">
        {[
          ...
          <div
            key={i}
            className={`rounded-lg border border-white/10 bg-slate-900/60 p-1.5 ${item.c}`}
          >
            {item.icon}
          </div>
        ))}
  ...
  return (
    <div className="flex items-center gap-2 w-full">
      ...
      <div className="flex flex-col items-center w-16 shrink-0">
        ...
        <div className="flex items-center w-full">
          <div
            className="flex-1 border-t-2 border-dashed border-red-500/70"
    ...
    <div className="flex items-stretch gap-3 w-full">
      <div className="flex-1 rounded-lg border border-fuchsia-500/30 bg-fuchsia-950/30 px-3 py-2 min-w-0">
        <div className="flex items-center gap-1.5 mb-1.5">
          ...
          <span className="text-[8px] font-bold uppercase tracking-widest text-fuchsia-400">
            AI Narrative
          </span>
        </div>
      ...
      <div className="flex flex-col items-center justify-center rounded-lg border border-red-500/40 bg-red-950/40 px-3 py-2 shrink-0 min-w-[100px]">
        <span className="text-[7px] uppercase tracking-widest text-red-400 mb-0.5">
          Prime Suspect
        </span>
...
export function TimelineReplay() {
  ...
  const { data: timelineData } = useTimeline("C-2041");
  useEffect(() => {
    if (timelineData?.timeline) setBackendEvents(timelineData.timeline);
  }, [timelineData]);
  ...
  return (
    <div
      className="relative flex flex-col overflow-hidden rounded-xl border border-white/8 bg-[#060a18]"
      style={{ height: "760px" }}
    >
      {/* Header */}
            ...
            <>
              <div className="hidden lg:flex items-center gap-3 border-r border-white/8 pr-3">
                <div className="flex items-center gap-1">
                  <div className="w-6 h-px bg-cyan-400/80" />
                  <span className="text-[8px] text-slate-400">Direct</span>
                </div>
                <div className="flex items-center gap-1">
                  <div className="w-6 h-px border-t border-dashed border-slate-400/70" />
                  <span className="text-[8px] text-slate-400">Indirect</span>
                </div>
                <div className="flex items-center gap-1">
                  <div
                    className="w-6 h-px border-t-2 border-dashed border-red-500/80"
                    style={{ boxShadow: "0 0 4px rgba(239,68,68,0.4)" }}
                  />
      ...
      {dataLoaded && (
        <div
          className="grid shrink-0 border-b border-white/6 bg-slate-950/40"
        ...
        >
          {[
            ...
            "TIME",
            "EVENT",
            "GRAPH STATE (EVIDENCE ADDED STEP BY STEP)",
            "AI INSIGHT / IMPACT",
          ].map((h) => (
            <div
              key={h}
              className="px-2 py-1.5 text-[8px] font-bold uppercase tracking-widest text-slate-600 border-r border-white/5 last:border-r-0"
            >
              {h}
            </div>
          ))}
      ...
      <div ref={bodyRef} className="flex-1 overflow-y-auto relative">
        {!dataLoaded ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div
              className="flex flex-col items-center gap-3 rounded-2xl border border-cyan-500/20 bg-slate-950/60 px-10 py-8 text-center"
            ...
            >
              ...
              <div>
                <div className="text-[12px] font-black uppercase tracking-[0.18em] text-cyan-400">
                  Upload CCTV & Log Data
                </div>
                <div className="mt-1.5 text-[10px] text-slate-500 max-w-[260px] leading-relaxed">
                  Upload CCTV footage and investigation logs to generate the 18-step timeline
                  reconstruction
                </div>
              </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start pt-2.5 px-2 border-r border-white/5">
                  <span
                    className={`font-mono text-[11px] font-bold ${isActive ? ec.text : isPast ? "text-slate-600" : "text-slate-500"}`}
                  >
                    {s.time}
                  </span>
                </div>
                <div className="flex flex-col justify-start px-2 py-2 border-r border-white/5 min-w-0">
                  <div
                    className={`text-[11px] font-semibold leading-tight mb-0.5 ${isActive ? "text-white" : isPast ? "text-slate-500" : "text-slate-400"}`}
                  >
                    {s.title}
                  </div>
                  <div
                    className={`text-[9px] leading-snug mb-1 ${isActive ? "text-slate-400" : "text-slate-600"} line-clamp-2`}
                  >
                    {s.description}
                  </div>
                  <div
                    className={`flex items-center gap-1 ${isActive ? ec.text : "text-slate-600"}`}
                  >
      ...
      {dataLoaded && (
        <div className="shrink-0 border-t border-white/6 px-4 py-1.5 flex items-center gap-3 bg-slate-950/50">
          ...
          <span
            className={`font-mono text-[9px] font-semibold ${step.severity === "critical" ? "text-red-400" : step.severity === "suspicious" ? "text-orange-400" : "text-emerald-400"}`}
          >
