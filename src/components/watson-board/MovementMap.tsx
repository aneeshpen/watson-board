...
import { heatmapZones, movementPath, crimeScene } from "@/data/data";
import { useDistrictFilter } from "@/contexts/DistrictFilterContext";
import { MapInteractChrome } from "@/components/watson-board/MapInteractChrome";
import { type MovementPoint } from "@/lib/api";
import { useMovement } from "@/hooks/use-api";
import { cn } from "@/lib/utils";

const MOVEMENT_DISTRICT = "Bengaluru";
...
function buildMap(container: HTMLDivElement, points: MovementPoint[]) {
  ...
  if (victim.length > 1)
    L.polyline(
      victim.map((p) => [p.lat, p.lng] as [number, number]),
      {
        color: "#5fd4ff",
        weight: 3,
        opacity: 0.9,
  ...
  if (suspect.length > 1)
    L.polyline(
      suspect.map((p) => [p.lat, p.lng] as [number, number]),
      {
        color: "#ff4d6d",
        weight: 3,
        opacity: 0.9,
  ...
  [...victim, ...suspect, ...crime].forEach((p) => {
    ...
    L.circleMarker([p.lat, p.lng], {
      ...
      .bindTooltip(`${p.label} · ${p.time}`, { direction: "top" })
      .addTo(map);
  });
...
function legacyToPoints(path: typeof movementPath): MovementPoint[] {
  return path.map((p) => ({
    lat: p.lat,
    lng: p.lng,
    label: p.who === "victim" ? "Victim position" : "Suspect position",
    time: p.t,
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <div className="text-[9px] text-slate-500 mt-0.5">
              CSV, JSON, or any format · routes auto-rendered
            </div>
          </div>
          ...
          >
            <div>
              ...
              <div className="text-[9px] text-slate-500">
                Victim · Suspect · Crime scene routes plotted
              </div>
            </div>
            <button
              onClick={onDismiss}
...
export function MovementMap() {
  ...
  // React Query: cached and deduplicated, so mounting this alongside other
  // consumers no longer fires the same request twice.
  const { data: movementData } = useMovement("C-2041");
  useEffect(() => {
    if (movementData?.movement) setPoints(movementData.movement);
  }, [movementData]);
  ...
  useEffect(() => {
    ...
    // Always place the crime scene marker for the primary case district
    if (!districtFilter || districtFilter === MOVEMENT_DISTRICT) {
      const csMarker = L.circleMarker([crimeScene.lat, crimeScene.lng], {
        radius: 11,
        color: "#f59e0b",
        fillColor: "#f59e0b",
        fillOpacity: 0.92,
      ...
      csMarker.bindTooltip(
        `<div style="font-size:11px"><b>Crime Scene</b><br/>${crimeScene.label}</div>`,
        { direction: "top" },
      );
      csMarker.addTo(map);
    }
    ...
    if (!showReplay && districtFilter) {
      ...
      else map.setView([15.3173, 75.7139], 7);
    }
  ...
  return (
    ...
    >
      ...
      {/* Legend — crime scene always shown; routes shown once data is loaded */}
      {(!districtFilter || districtFilter === MOVEMENT_DISTRICT) && (
        <motion.div
        ...
        >
          <span className="flex items-center gap-1.5">
            <span
              className="h-2.5 w-2.5 rounded-full shadow-[0_0_8px_#f59e0b]"
              style={{ backgroundColor: "#f59e0b" }}
            />
            <span className="font-semibold text-amber-400">Crime Scene</span>
          </span>
          {dataLoaded && (
            <>
              <span className="flex items-center gap-1.5">
                <span
                  className="h-2 w-2 rounded-full shadow-[0_0_8px_#5fd4ff]"
                  style={{ backgroundColor: "#5fd4ff" }}
                />{" "}
                Victim
              </span>
              <span className="flex items-center gap-1.5">
                <span
                  className="h-2 w-2 rounded-full shadow-[0_0_8px_#ff4d6d]"
      ...
      <AnimatePresence>
        {!dataLoaded && !isSimulating && (
          ...
          >
            <div
              className="flex flex-col items-center gap-3 rounded-2xl border border-cyan-500/20 bg-black/70 px-8 py-5 backdrop-blur-sm text-center"
              style={{ boxShadow: "0 0 40px rgba(34,211,238,0.06)" }}
            >
              <div
                className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-500/30 bg-cyan-950/50"
                style={{ boxShadow: "0 0 20px rgba(34,211,238,0.15)" }}
              >
                <MapPin className="h-5 w-5 text-cyan-400" />
              </div>
              <div>
                <div className="text-[11px] font-black uppercase tracking-[0.18em] text-cyan-400">
                  Import GPS Log Data
                </div>
                <div className="mt-1 text-[10px] text-slate-500 max-w-[200px] leading-relaxed">
                  Crime scene plotted. Upload GPS log data to overlay movement routes.
                </div>
              </div>
            </div>
          </motion.div>
      ...
      {!showReplay && districtFilter && dataLoaded && (
        <div className="pointer-events-none absolute bottom-3 left-3 right-16 z-[500] rounded-md border border-primary/35 bg-background/85 px-3 py-2 text-center text-[11px] text-muted-foreground backdrop-blur">
          Movement replay is only modeled for{" "}
          <span className="font-medium text-foreground">{MOVEMENT_DISTRICT}</span> (C-2041). Map
          centered on <span className="font-medium text-foreground">{districtFilter}</span>.
        </div>
      )}
